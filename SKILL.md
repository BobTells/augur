...
For a worked example of the whole file (~20 lines), see [`REFERENCE.md`](REFERENCE.md).
...
Use this when Step 2 returned no input, **or** the user picked "interview-heavy" or "combine" in Step 0 / Step 2.

Run the [[grill-me]] skill. **Each question narrows on the prior answer** — don't pre-plan a fixed set; conduct an interview of a length and depth appropriate to this user. Confirm readiness first, and fall back to a manual `AskUserQuestion` loop if grill-me isn't installed. Full mechanics + the readiness prompt → [`REFERENCE.md`](REFERENCE.md).
